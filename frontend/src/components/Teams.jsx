...
export default function Teams() {
  ...
  if (loading) return (
    <div className="h-screen bg-[#080a0f] flex flex-col items-center justify-center gap-4">
      ...
      <span className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">Syncing Team Database</span>
    </div>
  );
        ...
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 mb-12">
          <div>
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">ICC Global Directory</span>
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
            ...
            <button
              ...
              className={`px-8 py-3 rounded-2xl font-black uppercase text-[10px] tracking-widest transition-all ${
                compareMode ? "bg-blue-600 text-white shadow-xl shadow-blue-600/20" : "bg-white/5 border border-white/5 text-slate-400"
              }`}
            >
          ...
          {["rank", "trophies", "name"].map(type => (
            <button
              ...
              className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${
                sortBy === type ? "bg-white text-black" : "text-slate-500 hover:text-slate-300"
              }`}
            >
              {type}
            </button>
            ...
            >
              <div className="bg-[#111827] border border-blue-500/30 text-white px-8 py-4 rounded-[2rem] shadow-2xl flex items-center gap-8 backdrop-blur-xl">
                <div className="flex flex-col">
                  <span className="text-[10px] font-black uppercase text-blue-500">Battle Selection</span>
                ...
                <button
                  ...
                  className={`px-8 py-2 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all ${
                    selectedTeams.length === 2 ? "bg-blue-600 hover:bg-blue-700" : "bg-slate-800 text-slate-600 cursor-not-allowed"
                  }`}
                >
                  Start Analysis
                </button>
                <button onClick={() => {setCompareMode(false); setSelectedTeams([]);}} className="text-slate-500 hover:text-red-500 transition-colors">
                  <X size={18} />
                </button>
              </div>
            </motion.div>
...
const TeamCard = ({ team, compareMode, isSelected, onSelect }) => (
  <motion.div 
    ...
    whileHover={!compareMode ? { y: -8, borderColor: "rgba(59, 130, 246, 0.4)" } : {}}
    onClick={compareMode ? onSelect : null}
    className={`relative bg-[#111a2e] p-8 rounded-[2.5rem] border transition-all cursor-pointer ${
      isSelected ? "border-blue-500 bg-blue-500/5 ring-4 ring-blue-500/10" : "border-white/5"
    }`}
  >
    {compareMode && (
